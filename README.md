...
### 版本 2.4 (2026-04-20)
- **拖拽路径解析优化**：修复了在 Windows 系统下拖拽含有空格、中文或特殊字符的文件夹/文件时，路径解析失败或带花括号 `{}` 的常见问题。
- **自定义模板增强**：新增了对更多 EXIF 变量的支持（如相机制造商、镜头型号等），并优化了无 EXIF 数据时的默认命名逻辑。
- **界面与交互微调**：优化了进度条显示，在重命名过程中提供更直观的进度反馈；调整了部分控件在不同分辨率下的排版。
- **打包配置调整**：更新了 PyInstaller 与 Nuitka 的打包脚本，缩减了打包后的体积并加快了启动速度。
